...
def load_admin_keys_from_env():
    ...
    Uses: X_ECM_API_ID, X_ECM_API_KEY, X_CP_API_ID, X_CP_API_KEY. TOKEN is optional - use as CP key if X_CP_API_KEY is empty.
    """
    ...
    admin_x_cp_api_key = (os.environ.get('X_CP_API_KEY') or os.environ.get('TOKEN') or '').strip()
    ...
    if all([admin_x_ecm_api_id, admin_x_ecm_api_key, admin_x_cp_api_id, admin_x_cp_api_key]):
        try:
            ...
            if accounts and len(accounts) > 0:
                # Keys loaded successfully; store for "use admin keys" option
                return admin_api_keys
        except Exception:
            # Silently fail if keys don't work
            pass
    return None

# Load admin keys on startup
_admin_keys_from_env = load_admin_keys_from_env()
# When from env, _admin_keys_from_env is dict with X-ECM-API-ID etc.; else None
...
def index():
    if request.method == 'POST':
        ...
        # Handle config keys form submission
            ...
            use_admin_keys = request.form.get('use_admin_keys') == '1'
            ...
            if use_admin_keys:
                # Use admin keys from session (or from env if session wasn't populated)
                if isinstance(_admin_keys_from_env, dict):
                    config_x_ecm_api_id = session.get('admin_x_ecm_api_id') or _admin_keys_from_env.get('X-ECM-API-ID', '')
                    config_x_ecm_api_key = session.get('admin_x_ecm_api_key') or _admin_keys_from_env.get('X-ECM-API-KEY', '')
                    config_x_cp_api_id = session.get('admin_x_cp_api_id') or _admin_keys_from_env.get('X-CP-API-ID', '')
                    config_x_cp_api_key = session.get('admin_x_cp_api_key') or _admin_keys_from_env.get('X-CP-API-KEY', '')
                else:
                    config_x_ecm_api_id = session.get('admin_x_ecm_api_id', '')
                    config_x_ecm_api_key = session.get('admin_x_ecm_api_key', '')
                    config_x_cp_api_id = session.get('admin_x_cp_api_id', '')
                    config_x_cp_api_key = session.get('admin_x_cp_api_key', '')
                config_bearer_token = ''
            ...
            # Validate inputs - require all APIv2 keys OR APIv3 token (or use_admin_keys)
            if use_admin_keys and not has_all_apiv2:
                session['config_error_message'] = 'Admin keys are not available. Please re-enter admin keys in Step 1.'
    ...
    if isinstance(_admin_keys_from_env, dict) and not session.get('admin_keys_set'):
        try:
            accounts = ncm.get_accounts()
            if accounts and len(accounts) > 0:
                session['admin_keys_set'] = True
                session['account_name'] = accounts[0].get('name', 'Unknown')
                # Store keys in session for "use admin keys" option
                session['admin_x_ecm_api_id'] = _admin_keys_from_env.get('X-ECM-API-ID', '')
                session['admin_x_ecm_api_key'] = _admin_keys_from_env.get('X-ECM-API-KEY', '')
                session['admin_x_cp_api_id'] = _admin_keys_from_env.get('X-CP-API-ID', '')
                session['admin_x_cp_api_key'] = _admin_keys_from_env.get('X-CP-API-KEY', '')
        except Exception:
            pass
    # Verify NCM instance is still valid if session says keys are set
    elif session.get('admin_keys_set'):
        try:
            # Try to get accounts to verify NCM instance exists
            ncm.get_accounts()
        except:
