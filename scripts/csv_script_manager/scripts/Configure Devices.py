...
Usage:
    python "Configure Devices.py"                    # Uses default router_grid.csv and API keys from code/env
    python "Configure Devices.py" <csv_file_path>    # Uses specified CSV file and API keys from code/env
...
def load_csv(filename):
    ...
    try:
        with open(filename, 'rt', encoding='utf-8-sig') as f:
            ...
            # e.g. id, router, routerid, router id, router_id
            ...
            if not id_column:
                print(
                    "Error: Could not find router ID column. "
                    f"Available columns: {', '.join(reader.fieldnames)}"
                )
                print(
                    "Expected one of: id, router, routerid, router id, router_id "
                    "(case-insensitive)."
                )
                return router_configs
