    ...
    with open('test_routers.csv', 'r') as f:
        ...
        with requests.Session() as s:
            ...
            for row, column in enumerate(config_csv):
                ...
                try:
                    # Fill in GRE info
                    indi_payload["configuration"][0]["gre"]["tunnels"]["00000000-adbb-3f47-962c-cd7ea9a221af"][
                        "local_network"] = gre_ip
                    indi_payload["configuration"][0]["gre"]["tunnels"]["00000001-3248-3c72-96d3-adb4353e0501"][
                        "local_network"] = gre_ip2

                    # Fill in LAN info
                    indi_payload["configuration"][0]["lan"]["00000000-0d93-319d-8220-4a1fb0372b51"][
                        "ip_address"] = local_network_ip
                    indi_payload["configuration"][0]["lan"]["00000000-0d93-319d-8220-4a1fb0372b51"][
                        "netmask"] = local_network_mask

                    # BGP config info
                    indi_payload["configuration"][0]["routing"]["bgp"]["routers"]["0"]["router_id"] = local_network_ip
                    indi_payload["configuration"][0]["routing"]["bgp"]["routers"]["0"]["networks"]["0"][
                        "ip_network"] = local_network_ip + '/27'
                    ...
                    print('Config patch sent to router {}\nResponse = {}\n'.format(router_id, patch_result))

                    # Catch failed status codes and log them
                    if patch_result is not 202:
                        print("Patch unsuccessful. Status code {}, Response: {}\n".format(patch_result, patch.text))
                        patch_text = patch.text

                # catch index errors, probably from a failed GET
                except IndexError as e:
                    print('{} IndexError, probably from failed GET. Exception = {}\nGET Response = {}\n'.format(router_id, e, cfg_id))
                ...
                # write patch result to csv
                    ...
                    with open('results.csv', 'a', newline='') as patch_results:
                        ...
                        patch_results_csv.writerow([router_id, patch_result, patch_text])
