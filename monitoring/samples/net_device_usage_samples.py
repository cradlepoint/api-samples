...
to poll the net_device_usage_samples endpoint for all new usage samples.
The sample process_usage_sample() function just prints any new metrics to the
...
ID_BATCH_LIMIT = 100  # request samples for max 100 IDs at a time
POLL_START_TIME = datetime.utcnow() - timedelta(hours=1)


def poll_for_new_samples(session=None, ids=[]):
    endpoint = "net_device_usage_samples"
...
def process_usage_sample(s):
    """Do whatever you need to with a single usage_sample."""
    pprint(s)
