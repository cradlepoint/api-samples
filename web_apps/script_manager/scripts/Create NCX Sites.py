...
Requirements:
    ...
    - API keys set as environment variables: X_ECM_API_ID, X_ECM_API_KEY,
      X_CP_API_ID, X_CP_API_KEY, NCM_API_TOKEN
"""
...
def create_ncx_sites(mode: str, rows: list[tuple[str, str, str | None]]) -> None:
    ...
    token = api_keys.get("token") or os.environ.get("NCM_API_TOKEN")
    if not token:
        print("Error: NCM_API_TOKEN is required for NCX v3 API (set in API Keys tab).")
        return
